...
class Metrics(object):
    ...
    def acc(self):
        return f'{(self._tp_tn() / self._sum()).item() * 100:.2f}'
    ...
    def miou(self):
        iou = self.iou()
        return iou.sum() / len(iou) 
    
    def iou(self):
        return torch.diagonal(self.cm, 0) / (self.cm.sum(dim=0) + self.cm.sum(dim=1) - torch.diagonal(self.cm, 0))
